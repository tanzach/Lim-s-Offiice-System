...
def create_customer_order(request):
    if request.user.is_authenticated: 
        ...
        if(request.method == "POST"): 
            ...
            product_id = request.POST.get('productname') # gets product key
...
def customer_order(request, pk):
    employee_type = request.user.Employee_Type
    c_order = Order.objects.filter(pk=pk)
    return render(request, 'projectapp/customer_order.html', {'employee_type':employee_type, 'customer_order':c_order})
...
def add_products(request):
    ...
    if(request.method == "POST"):
        ...
        product = Inventory.objects.filter(Product_Name=product_name, Unit=unit, Unit_Price=unit_price, Acquisition_Code=ac_code, Warehouse_Location=location, Supplier=supplier_name)
